    ...
    <div className="space-y-6 animate-fade-in pb-24">
      ...
      {isLoading ? <Loader /> : error ? (
        ...
        <div className="space-y-4">
          <div className="rounded-xl sm:rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900/60 shadow-xs overflow-hidden">
            <div className="w-full overflow-x-auto">
              <table className="w-full text-left text-sm text-slate-500 dark:text-slate-400 table-auto">
                <thead className="border-b border-slate-200 bg-slate-50 text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:border-slate-800 dark:bg-slate-800/60 dark:text-slate-300">
                  <tr>
                    ...
                    <th scope="col" className="px-2 sm:px-3 py-3">{t('users.columnContact')}</th>
                    ...
                    return (
                      <tr key={user._id} className="transition-colors hover:bg-slate-50/70 dark:hover:bg-slate-800/40">
                        <td className="px-3 sm:px-4 py-2.5">
                          <div className="flex items-center gap-2 min-w-0" title={t('users.joined', { date: joinedDate })}>
                            ...
                            <span className="font-semibold text-xs sm:text-sm text-slate-900 dark:text-white truncate max-w-[120px] sm:max-w-[160px] flex items-center gap-1">
                        ...
                        <td className="px-2 sm:px-3 py-2.5 font-mono text-xs font-semibold text-slate-700 dark:text-slate-300 whitespace-nowrap">{user.studentId || '—'}</td>
                        <td className="px-2 sm:px-3 py-2.5">
                          <div className="flex items-center gap-1.5 text-xs max-w-[170px] sm:max-w-[210px] truncate" title={user.email}>
                            ...
                            {user.phone && (
                              <span className="hidden xl:inline text-slate-400 shrink-0 text-[10px]" title={user.phone}>
                                ({user.phone})
                              </span>
                            )}
                          </div>
                        ...
                        <td className="px-2 sm:px-3 py-2.5 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold ${user.role === 'admin' ? 'bg-indigo-50 text-indigo-700 border border-indigo-200/60 dark:bg-indigo-950/40 dark:text-indigo-300 dark:border-indigo-800/50' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300'}`}>
