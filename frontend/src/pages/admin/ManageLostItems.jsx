...
import { HelpCircle, Trash2, Calendar, MapPin, Tag } from 'lucide-react';
...
const ManageLostItems = () => {
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      ...
      toast.success('Lost report successfully deleted.');
    } catch (err) {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Moderate, review, or delete lost item reports posted by users.
        </p>
      </div>
      ...
      <SearchFilter 
        ...
        statusOptions={lostStatusOptions}
        onClear={() => {
          setSearch('');
          setCategory('');
          setStatus('');
      ...
      {isLoading ? (
        ...
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 p-4 rounded-xl text-sm">
          Failed to fetch lost reports: {error}
        </div>
      ) : items.length === 0 ? (
        <EmptyState 
          title="No Lost Items Found" 
          message="No lost reports match your search query." 
        />
      ) : (
      ...
      {deleteId && (
        <ConfirmDialog 
          ...
          message="Are you sure you want to permanently delete this lost item report? This action is irreversible and will delete any associated match listings."
          confirmText="Delete"
          variant="danger"
        />
      )}
