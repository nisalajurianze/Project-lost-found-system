...
export const MyLostItems = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(t('myItems.lostDeleted'));
      setDeleteDialogOpen(false);
    } catch (err) {
      toast.error(err || t('myItems.deleteReportError'));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {items.length === 0 ? (
        <EmptyState
          title={t('myItems.emptyLostTitle')}
          description={t('myItems.emptyLostDesc')}
        />
      ) : (
        <>
              ...
              <table className="table-base w-full text-sm text-left whitespace-nowrap min-w-[700px]">
                <thead className="table-head bg-surface-50 dark:bg-surface-800 text-surface-600 dark:text-surface-400">
                  <tr>
                    ...
                    <th className="px-6 py-4">{t('myItems.dateLost')}</th>
                  ...
                  {items.map((item) => (
                    <tr key={item._id} className="table-row border-t border-surface-200 dark:border-surface-700 hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-surface-500 dark:text-surface-400">
                        {new Date(item.lostDate).toLocaleDateString(language === 'si' ? 'si-LK' : language === 'ta' ? 'ta-LK' : 'en-US')}
                      </td>
      ...
      {feedbackDialog && (
        <FeedbackModal
          ...
          defaultSubject={t('myItems.lostFeedbackSubject', { item: feedbackDialog.itemName })}
        />
      )}
      ...
      <ConfirmDialog
        ...
        message={t('myItems.deleteLostConfirm')}
