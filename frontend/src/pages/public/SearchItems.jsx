    ...
    <div className="flex-1 bg-surface-50 py-8 dark:bg-surface-900">
      <div className="page-container mx-auto max-w-screen-2xl">
        ...
        <section aria-label={t('common.search')} className="rounded-2xl border border-surface-200 bg-white p-4 shadow-sm dark:border-surface-800 dark:bg-surface-950">
          ...
          {activeFilters.length > 0 && <div className="mt-4 flex flex-wrap items-center gap-2" aria-label={t('search.filters')}>{activeFilters.map((filter) => <button key={filter.key} type="button" onClick={() => clearFilter(filter.key)} className="inline-flex min-h-9 items-center gap-1 rounded-full bg-primary-50 px-3 text-sm font-semibold text-primary-800 dark:bg-primary-950/50 dark:text-primary-200">{filter.label}<FiX aria-hidden="true" /></button>)}<button type="button" onClick={clearAll} className="min-h-9 px-2 text-sm font-bold text-primary-700 underline dark:text-primary-300">{t('search.clear')}</button></div>}
        ...
        <div className="my-6 flex flex-wrap items-center justify-between gap-3"><p role="status" className="font-semibold text-surface-700 dark:text-surface-200">{isLoading && page === 1 ? t('search.loading') : t('search.results', { count: total })}</p><div className="flex gap-2"><Link to="/dashboard/report-lost" className="btn btn-outline btn-sm"><FiPlusCircle />{t('nav.reportLost')}</Link><Link to="/dashboard/report-found" className="btn btn-primary btn-sm"><FiPlusCircle />{t('nav.reportFound')}</Link></div></div>
