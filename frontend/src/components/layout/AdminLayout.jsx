  ...
  return (
    <div className="min-h-screen flex flex-col bg-surface-50 dark:bg-surface-900 transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col lg:flex-row w-full relative">
        ...
        {/* Mobile Admin Header & Dropdown */}
        <div className="lg:hidden">
          <div className="flex items-center justify-between p-4 bg-white dark:bg-surface-800 border-b border-surface-200 dark:border-surface-700/50 shadow-sm relative z-20">
            <span className="font-bold font-display text-surface-900 dark:text-white tracking-wide">
              Admin Navigation
            </span>
            <button 
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} 
              className="p-2 rounded-xl bg-surface-100 dark:bg-surface-700 text-surface-600 dark:text-surface-300 hover:bg-surface-200 dark:hover:bg-surface-600 transition-colors focus:outline-none"
            >
          ...
          <div className={`absolute left-0 right-0 z-10 bg-white/95 dark:bg-surface-800/95 backdrop-blur-xl border-b border-surface-200 dark:border-surface-700/50 shadow-xl transition-all duration-300 origin-top overflow-hidden ${isMobileMenuOpen ? 'max-h-[70vh] opacity-100' : 'max-h-0 opacity-0 pointer-events-none'}`}>
