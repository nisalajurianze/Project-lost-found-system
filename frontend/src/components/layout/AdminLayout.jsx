...
export const AdminLayout = () => {
  ...
  const renderLink = (link, mobile = false) => (
    <NavLink
      ...
      className={({ isActive }) => {
        if (!mobile) {
          return `${isActive ? 'sidebar-link-active' : 'sidebar-link'} ${link.borderTop ? 'mt-2.5 pt-2.5 border-t border-surface-200/80 dark:border-surface-800/80 !rounded-xl' : ''}`;
        }
        return `${isActive ? 'bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 font-bold' : 'text-surface-600 dark:text-surface-300 hover:bg-surface-50 dark:hover:bg-surface-800/40'} flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-colors ${link.borderTop ? 'mt-2 pt-2 border-t border-surface-200/70 dark:border-surface-800/70' : ''}`;
      }}
    >
      <span className="text-[1.15rem] opacity-90 shrink-0">{link.icon}</span>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-surface-50 dark:bg-surface-900 transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col lg:flex-row w-full relative">
        ...
        <aside className="w-[275px] flex-shrink-0 border-r border-surface-200/50 bg-white/40 dark:border-surface-800/50 dark:bg-surface-950/40 backdrop-blur-xl p-4 hidden lg:flex flex-col no-print sticky top-16 h-[calc(100vh-4rem)]">
          <div className="mb-3 flex items-center justify-between rounded-xl border border-primary-500/20 bg-primary-500/10 px-3 py-2">
            <div className="flex items-center gap-2">
              <span className="flex h-2 w-2 rounded-full bg-primary-500 animate-pulse" />
              <span className="text-[11px] font-bold tracking-wider text-primary-700 dark:text-primary-300 uppercase">
                {t('admin.panelLabel')}
              </span>
            </div>
            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-primary-500/20 text-primary-800 dark:text-primary-200">
              Admin
            </span>
          </div>

          <nav aria-label={t('admin.panelLabel')} className="flex flex-col gap-0.5 flex-1 overflow-y-auto custom-scrollbar pr-1">
            {adminLinks.map((link) => renderLink(link))}
          </nav>

          <div className="mt-auto pt-3 border-t border-surface-200/50 dark:border-surface-800/50">
            <button
              type="button"
              onClick={handleLogout}
              className="w-full min-h-11 flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl text-sm font-semibold text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/25 transition-all focus:outline-none focus:ring-2 focus:ring-red-500/50"
            >
              <FiLogOut className="text-[1.15rem] opacity-90 shrink-0" aria-hidden="true" />
