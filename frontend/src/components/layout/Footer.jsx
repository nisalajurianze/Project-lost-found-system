    ...
    <footer className="w-full border-t border-surface-200/50 bg-white dark:border-surface-800/50 dark:bg-surface-950/80 transition-colors duration-300 py-8 no-print mt-auto pb-24 lg:pb-8">
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap justify-center gap-6 text-sm text-surface-600 dark:text-surface-400 font-medium">
            <Link to="/about" className="hover:text-primary-500 transition-colors">{t('common.about')}</Link>
            <Link to="/contact" className="hover:text-primary-500 transition-colors">{t('common.contact')}</Link>
            <Link to="/lost-items" className="hover:text-primary-500 transition-colors">{t('nav.searchItems')}</Link>
            <Link to="/dashboard/report-found" className="hover:text-primary-500 transition-colors">{t('nav.reportFound')}</Link>
          </div>
        </div>
