...
export const Loader = ({ size = 'md', fullPage = false, className = '' }) => {
  ...
  if (fullPage) return <div className="flex h-[60vh] w-full flex-col items-center justify-center gap-6" role="status" aria-label={t('loader.application')}><img src="/logo.png" alt="" aria-hidden="true" className="inline-block h-16 w-16 animate-search-circle object-contain sm:h-20 sm:w-20" /><span className="animate-pulse text-sm font-medium text-surface-500 dark:text-surface-400">{t('loader.application')}</span></div>;
  return spinner;
};
