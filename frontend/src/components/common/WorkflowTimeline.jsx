...
const WorkflowTimeline = ({ type = 'item', status, contactShared = false, className = '' }) => {
  ...
  return (
    <section className={`min-w-0 rounded-2xl border border-surface-200 bg-white p-4 dark:border-surface-800 dark:bg-surface-900/60 ${className}`} aria-label={t('workflow.timeline')}>
      <h3 className="text-sm font-extrabold uppercase tracking-wider text-surface-500">{t('workflow.timeline')}</h3>
      <ol className="mt-4 grid grid-cols-[repeat(auto-fit,minmax(min(100%,11rem),1fr))] gap-3" aria-label={t('workflow.progress')}>
        {steps.map((step, index) => {
          ...
          return <li key={step.key} className={`relative min-w-0 rounded-xl border p-3 ${complete ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-900/50 dark:bg-emerald-950/20' : current ? 'border-primary-300 bg-primary-50 dark:border-primary-900/50 dark:bg-primary-950/20' : 'border-surface-200 bg-surface-50 dark:border-surface-800 dark:bg-surface-950/30'}`}>
            <div className="flex min-w-0 items-center gap-2">
              {complete ? <CheckCircle2 className="h-5 w-5 shrink-0 text-emerald-600" aria-hidden="true" /> : current ? <Clock3 className="h-5 w-5 shrink-0 text-primary-600" aria-hidden="true" /> : <Circle className="h-5 w-5 shrink-0 text-surface-400" aria-hidden="true" />}
              <span className="min-w-0 break-words text-sm font-bold leading-snug">{t(`workflow.${step.key}`)}</span>
            </div>
          </li>;
        })}
      </ol>
