  ...
  return (
    <div className="card p-6 border-l-4 border-l-primary-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
        {/* Lost Item */}
        <div className="p-4 rounded-xl bg-surface-50 dark:bg-surface-900/30 border border-surface-200/50 dark:border-surface-800/50">
          ...
          <Link to={`/lost-items/${lost._id}`} className="hover:underline">
            <h5 className="text-sm font-bold text-surface-900 dark:text-white mt-1">
              {lost.itemName}
            </h5>
          </Link>
          ...
          <div className="mt-3 flex flex-col gap-1 text-[11px] text-surface-500 dark:text-surface-400">
            <p>📍 Location: <strong>{lost.lostLocation}</strong></p>
            <p>📅 Date: <strong>{new Date(lost.lostDate).toLocaleDateString()}</strong></p>
            <p>👤 Reported By: <strong>{lost.userId?.fullName || 'Me'}</strong></p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-surface-50 dark:bg-surface-900/30 border border-surface-200/50 dark:border-surface-800/50">
          ...
          <div className="mt-3 flex flex-col gap-1 text-[11px] text-surface-500 dark:text-surface-400">
            <p>📍 Location: <strong>{found.foundLocation}</strong></p>
            <p>📅 Date: <strong>{new Date(found.foundDate).toLocaleDateString()}</strong></p>
            <p>👤 Reported By: <strong>{found.userId?.fullName || 'Me'}</strong></p>
          </div>
        </div>
      </div>
      ...
      {match.status === 'confirmed' && (
        <div className="flex justify-between items-center mt-6 border-t border-surface-100 dark:border-surface-700/50 pt-4">
          ...
          <Link to={`/dashboard/report-found`} className="btn btn-primary btn-sm rounded-lg flex items-center gap-1.5">
            <FiLink /> File Ownership Claim
          </Link>
        </div>
      )}
    </div>
