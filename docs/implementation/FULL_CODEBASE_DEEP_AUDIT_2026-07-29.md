...
| High | Public Vercel `/api/*` requests were caught by the SPA fallback and returned `index.html`, leaving the healthy Railway API disconnected from the production UI. | Added ordered external rewrites for same-origin `/api/*` and Socket.IO polling to the verified Railway service before the SPA catch-all; regression coverage locks the order and destination. |
