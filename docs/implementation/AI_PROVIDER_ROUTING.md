...
- Report image acceptance is fail-closed at the backend as well as in the wizard. Every new image must receive an explicit provider result with `isItemPhoto=true`, `isSpam=false`, `moderationDecision=allow`, and `imageQuality` of `fair` or `good`; missing, uncertain, unsafe, explicit, adult, violent, unrelated, or provider-unavailable results are rejected before Cloudinary upload. This server gate also protects direct API callers that bypass the browser.
