...
The current Vercel deployment therefore keeps the browser on same-origin `/api` and `/socket.io` paths. Ordered external rewrites in `frontend/vercel.json` proxy those paths to the Railway backend before the SPA fallback. The exact `/socket.io/` trailing-slash handshake and `/socket.io` base rules must remain before the wildcard because the initial polling handshake has no wildcard segment. Do not move the catch-all rewrite above the API or Socket.IO rules; doing so returns `index.html` for service requests and breaks authentication or realtime delivery.
